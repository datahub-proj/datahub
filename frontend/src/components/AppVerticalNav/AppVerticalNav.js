...
import AppVerticalNavExpansionPanel from "./AppVerticalNavExpansionPanel";
...
export const AppVerticalNav = ({ items }) => {
  ...
  const renderLevels = (data) => {
    return data.map((item, index) => {
      ...
      if (item.children) {
        return (
          <AppVerticalNavExpansionPanel mode={mode} item={item} key={index}>
            {renderLevels(item.children)}
          </AppVerticalNavExpansionPanel>
        );
